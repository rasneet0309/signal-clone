"""
Fills the database with demo users, conversations, and messages.

Can be run two ways:
1. Manually, once, during local setup:   python -m app.seed
2. Automatically by main.py on server startup, but ONLY if the database
   is completely empty - this matters for free-tier hosting (like Render's
   free plan) where the disk is wiped whenever the service restarts, so
   the app always has demo data ready without anyone needing shell access.
"""
