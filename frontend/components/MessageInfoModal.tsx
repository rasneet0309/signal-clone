...
function StatusLine({ status }: { status: string }) {
  ...
  if (status === "delivered")
    return (
      <span className="flex items-center gap-1 text-ink-muted dark:text-zinc-400 text-xs font-medium">
        <CheckCheck size={14} /> Delivered
      </span>
    );
  return (
    <span className="flex items-center gap-1 text-ink-faint dark:text-zinc-500 text-xs font-medium">
      <Clock size={12} /> Sent
    </span>
  );
}
...
export default function MessageInfoModal({ messageId, onClose }: Props) {
  ...
  return (
    <Modal title="Message info" onClose={onClose}>
      ...
      {!statuses && !error && (
        <p className="text-sm text-ink-faint dark:text-zinc-500 text-center py-4">Loading...</p>
      )}
      {statuses && statuses.length === 0 && (
        <p className="text-sm text-ink-faint dark:text-zinc-500 text-center py-4">
          ...
          {statuses.map((s) => (
            ...
            >
              ...
              <div className="flex-1">
                <p className="text-sm font-medium dark:text-zinc-100">{s.user.display_name}</p>
                <p className="text-xs text-ink-faint dark:text-zinc-500">{formatTime(s.updated_at)}</p>
              </div>
